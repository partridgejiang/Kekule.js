...
ContentScriptNativeServicesImpl = {
	...
	doLoadFileData: function(doc, callback, options)
	{
		...
		MsgUtils.emitRequest(addon.port, globalConsts.MSG_LOAD_FILE_DATA, {
			'data': {'callerId': id, 'options': options},
			'responseCallback': function(data)
			{
				if (!data.result && data.errorMsg)  // fail to open, error occurs?
					Kekule.error(data.errorMsg);
				//console.log('response load file data', data);
				var id = data.callerId;
				var callback = ContentScriptNativeServicesImpl.getCallback(id);
				if (callback)
				{
					callback(data.result, data.data, data.fileName);
	...
	doSaveFileData: function(doc, data, callback, options)
	{
		...
		MsgUtils.emitRequest(addon.port, globalConsts.MSG_SAVE_FILE_DATA, {
			'data': {'callerId': id, 'data': data, 'options': options},
			'responseCallback': function(data)
			{
				if (!data.result && data.errorMsg)  // fail to open, error occurs?
					Kekule.error(data.errorMsg);
				...
				if (callback)
				{
					callback(data.result, data.fileName);
					ContentScriptNativeServicesImpl.unregisterCallback(id);
				}
			}
		});
	...
	{
		/*
		addon.port.on(globalConsts.MSG_LOAD_FILE_DATA_RESPONSE, function(msg)
		{
			var id = msg.callerId;
			var callback = ContentScriptNativeServicesImpl.getCallback(id);
			if (callback)
		...
		*/
		/*
		addon.port.on(globalConsts.MSG_SAVE_FILE_DATA_RESPONSE, function(msg)
		{
			var id = msg.callerId;
			var callback = ContentScriptNativeServicesImpl.getCallback(id);
			if (callback)
